...
Проект представляет сервер для проверки статуса кнопки роборуки. Статус переключается через URL-команды в адресной строке браузера: `/status_on` и `/status_off`.
